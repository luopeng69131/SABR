// g++ dp_my_rep.cc -o ../dp_my
// debug g++ -g -O0 -D_GLIBCXX_DEBUG dp_my_rep.cc -o dp_my_rep
#include "config.h"
#include <iostream>
#include <fstream>
#include <stdio.h>
...
// static const int VIDEO_BIT_RATE[] = {1000,2500,5000,8000,16000,40000};  // Kbps
unsigned int DEFAULT_QUALITY = 1;
unsigned int MAX_QUALITY = 5;
// float REBUF_PENALTY = 4.3;  // 1 sec rebuffering -> 3 Mbps
// float REBUF_PENALTY = 40;
int SMOOTH_PENALTY = 1;
float INVALID_DOWNLOAD_TIME = -1;

std::string COOKED_TRACE_FOLDER = TEST_TRACES;//"./video_trace/trace/4g_retrain/test/";
std::string OUTPUT_FILE_PATH = LOG_FILE_DIR + "log_sim_dp";//"./test_results/log_sim_dp";
...
void split(const std::string &s, std::vector<std::string> &elems) {
    std::regex re("\\s+"); // 匹配一个或多个空白字符（包括空格、制表符等）
    std::sregex_token_iterator it(s.begin(), s.end(), re, -1);
    std::sregex_token_iterator end;
    for (; it != end; ++it) {
        if (!it->str().empty()) { // 忽略空字符串（比如连续分隔符产生的空项）
            elems.push_back(*it);
        }
    }
}
        ...
        while ((entry = readdir(dir)) != NULL) {
            ...
            if (in_file.is_open()) {
                while (getline(in_file, line)) {
                    ...
                    if (parse.size() >= 2) {  // 确保解析的字段足够
                        cooked_time.push_back(std::stof(parse[0]));
                        cooked_bw.push_back(std::stof(parse[1]));
                        
                    } else {
                        printf("Warning: line %d in %s has less than 2 fields.\n", line_count, file_name.c_str());
                    }
                }
                in_file.close();

                // // 检查行数与 cooked_time 元素数量是否相等
                if (line_count != cooked_time.size()) {
                    printf("Mismatch in %s: line count = %d, cooked_time size = %lu\n", file_name.c_str(), line_count, cooked_time.size());
                } 
	...
	for(int bit_rate = 0; bit_rate < BITRATE_LEVELS; bit_rate ++) {
		...
		if (in_file.is_open())
		{
			...
			{   
                ...
                line_count++;
                if (line_count >= TOTAL_VIDEO_CHUNCK + 1 ) break;
			}
			in_file.close();
		}
...
int main() {
	...
	for (unsigned int trace_idx = 0; 
        ...
        std::cout << "all_cooked_time_bw.all_file_names[trace_idx]: " << all_cooked_time_bw.all_file_names[trace_idx] << std::endl;
        std::cout << "t_portion: " << t_portion << std::endl;
        std::cout << "t_max: " << t_max << std::endl;
        std::cout << "t_max_idx: " << t_max_idx << std::endl;
        std::cout << "cooked_time[cooked_time.size() - 1]: " << cooked_time[cooked_time.size() - 1] << std::endl;
		...
		float chunk_finish_time = quan_time[0] + download_time + LINK_RTT / M_IN_K;
		unsigned int time_idx = floor((chunk_finish_time - quan_time[0]) / DT);
		unsigned int buffer_idx = int(VIDEO_CHUNCK_LEN / M_IN_K / DT);

		float reward = VIDEO_BIT_RATE[DEFAULT_QUALITY] / M_IN_K \
			- REBUF_PENALTY * (chunk_finish_time - quan_time[0]) ;
        ...
        // lpc:  other alg has 48 rewards;
        // lpc: loop: 47 reward + init: 1 reward
		for (unsigned int n = 1; n < TOTAL_VIDEO_CHUNCK; n ++) {
			...
			for (auto it = total_reward[n-1].begin(); it != total_reward[n-1].end(); it ++) {
				...
				for (unsigned int nm = 0; nm < BITRATE_LEVELS; nm ++) {
					// printf("t num = %llu \n", t);
					// if (t >= all_download_time[n].size()) {
        			// 	continue;  // 超出索引范围，跳过当前迭代；
					// 	// deal index out of bound
    				// }
					download_time = all_download_time[n][t][nm];

                    //lpc： if exist quan_time[0] (start time), so must reduce it.
					float buffer_size = quan_time[b] - quan_time[0]; 
					float rebuf = download_time - buffer_size;
					if (rebuf < 0) rebuf = 0;
                    
                    // if (n < 3) {
                    //     std::cout << "start buffer_size: " << buffer_size << std::endl;
                    //     std::cout << "download_time: " << download_time << std::endl;
                    //     std::cout << "start quan t: " << quan_time[t] << std::endl;
                    //     std::cout << "----------------- " << std::endl;
                    // }
					...
					time_idx = floor((chunk_finish_time - quan_time[0]) / DT);
					// if (time_idx >= all_download_time[n].size()) {
					// 	time_idx = time_idx % all_download_time[n].size();
        			// 	// continue;  // 超出索引范围，跳过当前迭代；
					// 	// deal index out of bound
    				// }
					...
					if (reward + max_reward_remaining_after_n >= max_reward_up_to_n - 0.01) {
						...
						if (updated) {
                            // if (n < 3) {
                            //     std::cout << "chunk time: " << chunk_finish_time << std::endl;
		...
		for (auto it = total_reward[TOTAL_VIDEO_CHUNCK-1].begin(); it != total_reward[TOTAL_VIDEO_CHUNCK-1].end(); it ++) {
			...
			if (tmp_total_reward > optimal_total_reward) {
					...
					// std::cout << "1 chunk_idx: " << dp_pt.chunk_idx << ", "
					// 		  << "time_idx: " << dp_pt.time_idx << ", "
					// 	      << "buffer_idx: " << dp_pt.buffer_idx << ", "
					// 		  << "bit_rate: " << dp_pt.bit_rate << std::endl;
		...
		// std::cout << "chunk_idx: " << dp_pt.chunk_idx << ", "
		// << "time_idx: " << dp_pt.time_idx << ", "
		// << "buffer_idx: " << dp_pt.buffer_idx << ", "
		// << "bit_rate: " << dp_pt.bit_rate << std::endl;
		std::ofstream output;
		...
		log_file << TOTAL_VIDEO_CHUNCK - 1 << '\t' \
				 ...
				 << quan_time[last_buff_idx] - quan_time[0]  << '\t' \
		...
		while (dp_pt.chunk_idx != 0 || dp_pt.time_idx != 0 ||
			...
			log_file << dp_pt.chunk_idx << '\t' \
					 ...
					 << quan_time[dp_pt.buffer_idx] - quan_time[0]  << '\t' \
