...
public:
    ...
    Environment(std::vector<std::vector<double>> all_cooked_time, 
	    std::vector<std::vector<double>> all_cooked_bw, int seed = 42, double buffer_w = 0.0, bool is_train = true, int mpc_future_n=5);
    ~Environment(){;};
	double get_download_time(int video_chunk_size, double predict_tput);
	...
	int get_optimal(int last_bit_rate, double predict_tput, int horizon);
	int optimal;
    bool is_train;
    int mpc_future_chunk_count;
