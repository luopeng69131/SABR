...
from sim_env.test_cenv_rmpc import ABREnv
from config import VIDEO_BIT_RATE, REBUF_PENALTY, LOG_FILE_DIR, TEST_TRACES
...
algo = 'rmpc' # bs (beamSearch) or  mfd 'mpc future band'
buffer_w = 0
...
def main():
    ...
    env = ABREnv((all_cooked_time, all_cooked_bw), random_seed=42, future_N=MPC_FUTURE_CHUNK_COUNT, buffer_w=buffer_w)
    net_env = env.net_env
    ...
    while True:  # serve video forever
        step += 1
        future_bandwidth, reward, end_of_video, info = env.step(bit_rate)
        (delay, sleep_time, buffer_size, rebuf, \
            video_chunk_size, next_video_chunk_sizes, \
            end_of_video, video_chunk_remain) = info 
        ...
        if end_of_video:
            ...
            bit_rate = env.net_env.get_optimal(bit_rate, future_bandwidth, 5)
